...
private:
    ...
    vec3d evaluate(std::vector<vec3d>& cpGrid, double u, double v);

    double B3(double u) { return (1 - u)*(1 - u)*(1 - u) / 6.0; }
    double B2(double u) { return (3*u*u*u - 6*u*u + 4) / 6.0; }
    double B1(double u) { return (-3*u*u*u + 3*u*u + 3*u + 1) / 6.0; }
    double B0(double u) { return u*u*u / 6.0; }
